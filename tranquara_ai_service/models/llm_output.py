...
class CrisisCheckResult(BaseModel):
    is_crisis: bool = Field(
        description="Whether the content shows signs of psychological crisis")
...
class MemoryCandidate(BaseModel):
    content: str = Field(
        description="Durable psychological insight written in first person")
    category: str = Field(
...
class MoodHighlight(BaseModel):
    ...
    date: str = Field(description="ISO date string")
    title: str = Field(
        description="Journal entry title in the target language")


class MoodOverview(BaseModel):
    average: float = Field(
        description="Average mood score across all entries, from 1 to 10")
    trend: str = Field(
        description="Overall mood trend: 'improving', 'declining', or 'stable'")
    data_points: list[MoodDataPoint] = Field(
        description="Mood score per journal entry, chronologically ordered")
    highest: MoodHighlight = Field(
        description="Entry with the highest mood score")
    lowest: MoodHighlight = Field(
        description="Entry with the lowest mood score")
...
class EmotionalHighlight(BaseModel):
    ...
    significance: str = Field(
        description="Why this entry is emotionally significant, in the target language")
...
class JournalPattern(BaseModel):
    ...
    category: str = Field(
        description="One of: triggers, patterns, coping, relationships, growth")
...
class PrepPackResult(BaseModel):
    ...
    key_themes: list[str] = Field(
        description="3-5 recurring topics across entries, in the target language")
